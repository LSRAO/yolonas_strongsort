...
def draw_bbox_with_info(image,person_bboxes_xyxy,person_confidence,class_names):
    ...
    for bbox,class_name,conf in zip(person_bboxes_xyxy,class_names,person_confidence):
        ...
        cv2.putText(result, text, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
...
def detection_in_video():
    ...
    while True:
        ...
        if ret:
            ...
            model_predictions  = list(yolo_nas.predict(frame)._images_prediction_lst)
            ...
            annotated_frame = draw_bbox_with_info(frame,
                                                  class_names=person_labels_names,
